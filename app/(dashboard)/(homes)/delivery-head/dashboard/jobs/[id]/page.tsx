...
export default function DeliveryHeadJobDetailPage() {
    ...
    return (
        <>
            <DashboardBreadcrumb title={job.jobTitle} text="Jobs" />
            <div className="p-6 max-w-6xl mx-auto space-y-6">
                    ...
                    <div className="flex items-start justify-between gap-4 flex-wrap">
                        <div className="flex items-start gap-3">
                            <Button variant="outline" size="icon" className="h-9 w-9 shrink-0 mt-0.5" asChild>
                                <Link href="/delivery-head/dashboard/jobs"><ArrowLeft className="h-4 w-4" /></Link>
                            </Button>
                            <div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-4">
                        <div className="rounded-xl border border-neutral-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5 space-y-3">
                            <h3 className="text-xs font-bold uppercase tracking-[0.08em] text-neutral-500 dark:text-neutral-400">Job Snapshot</h3>
                            <InfoCard icon={DollarSign} label="Client Bill Rate" value={job.clientBillRate ? `$${job.clientBillRate}` : "-"} />
                            <InfoCard icon={DollarSign} label="Pay Rate" value={job.payRate ? `$${job.payRate}` : "-"} />
                            <InfoCard icon={Clock} label="Requirement Type" value={(job.requirementType || "").replace(/_/g, " ")} />
                            <InfoCard icon={Zap} label="Urgency" value={job.urgency || "-"} />
